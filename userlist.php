...
if($status==false){
  ...
  //Selectデータの数だけ自動でループ kanri_flg,life_flgを識別
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){

    if($result['kanri_flg']==1){
    $kanri = '管理者';
    }else if($result['kanri_flg']==0) {
    $kanri = '一般ユーザー';
    }
    if($result['life_flg']==1){
    $life = '退会';
    }else if($result['life_flg']==0){
    $life = '利用中';
    }
    $view .= '<hr><h5>'.$result['name'].' ';
    $view .= '<small><a href="userlist_edit.php?id='.$result['id'].'">編集'.'</a></small>'.'</h5>';
    $view .= '<div class="d-flex bd-highlight">';
    $view .= '<div class="p-2 bd-highlight"><img src="'.$result['icon'].'" width="40px"></div>';
    $view .= '<div class="p-2 flex-grow-1 bd-highlight">'.$kanri.' / '.$life.'</hr></div></div>';
  }
}
